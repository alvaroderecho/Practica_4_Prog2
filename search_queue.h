...
/**
 * @brief Data type for a pointer to a function that prints a SearchQueue element. 
 **/
typedef int (*p_squeue_ele_print)(FILE *, const void *);

/**
 * @brief Data type for a pointer to a function that prints a SearchQueue element. 
 **/
typedef int (*p_squeue_ele_cmp)(const void *, const void *);

/* START [_queue_new] */
/**
 * @brief This function creates and initializes a SearchQueue.
 *
 * Allocates memory for the new SearchQueue. When creating a SearchQueue it is necessary to
 ...
 *
 * @return A pointer to the SearchQueue if it could be created or NULL in case of error.
 *  */
SearchQueue * search_queue_new (p_squeue_ele_print print_ele, p_squeue_ele_cmp cmp_ele);
/* END [_queue_new] */

/**
 * @brief This function frees the memory used by the SearchQueue, but not the elements 
 * that it may contain.
 *
 ...
 * a SearchQueue. In particular, it will always return the smallest element according
 * to the comparison function passed in search_queue_new.
 *
 ...
 * position of a SearchQueue; this element should correspond with the largest element
 ...
 * position of a SearchQueue; this element should correspond with the smallest element
 * according to the comparison function passed in search_queue_new. 
 *
 * @param q A pointer to the SearchQueue.
 *
...
/**
 * @brief This function returns the size of a SearchQueue. Note that the function returns
 * 0 if it is called with a NULL pointer.
 *
 * @param q A pointer to the SearchQueue.
 *
